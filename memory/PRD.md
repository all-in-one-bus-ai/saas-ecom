...
- 2026-05-11: Stripe wired for both subscription billing AND shopper checkout. FastAPI backend at /app/backend/server.py hosts /api/payments/subscription/checkout, /api/payments/shop/checkout, /api/payments/status/{id}, /api/webhook/stripe — built on emergentintegrations.payments.stripe.checkout with platform key sk_test_emergent. Per-tenant Stripe key override supported via store_settings (stripe_secret_key / stripe_publishable_key) — falls back to platform key. payment_transactions ledger in MongoDB; on success, Supabase 'tenants.plan' updated for subscriptions OR new orders + order_items inserted for shopper carts. Frontend additions: Pricing section on landing (4 tiers + monthly/annual toggle), localStorage hybrid cart (lib/cart/use-cart.ts), CartIcon w/ live badge, AddToCartButton on product cards, /[tenantSlug]/cart page with checkout, /[tenantSlug]/checkout/success polling page, /store/[tenantSlug]/admin/billing with upgrade buttons + own-Stripe-keys form, /billing/success polling page. 22/22 tests passed (8 backend + 14 frontend).
- 2026-05-11: Multi-currency support (31 currencies). Default platform currency now GBP (£). New `/saas/settings` Default currency dropdown (super admin) and new `/store/[slug]/admin/settings` Currency dropdown (tenant admin) with live preview. Stripe checkout sessions use platform currency for subscription billing and tenant currency for shopper checkout. All price displays — landing pricing section, storefront product cards, cart, checkout success, billing page, super-admin subscriptions/analytics/activity/tenant detail, store-admin dashboard/products/orders/customers, operative dashboard — converted to formatPrice(amount, currency) via lib/currency.ts (Intl.NumberFormat). Backend exposes /api/payments/tenant-currency/{slug}. 22/22 tests passed.
